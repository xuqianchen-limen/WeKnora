...
WeKnora 的数据源导入模块支持从外部平台（飞书、Notion、Confluence 等）自动导入和同步内容到知识库。用户可配置数据源连接，选择需要同步的资源，并通过手动触发或定时调度自动完成内容的增量/全量同步。
...
| 企业微信文档 (wecom_doc) | `corp_id`, `agent_secret` | OAuth2 |
...
飞书连接器支持从飞书知识库（Wiki）同步文档到 WeKnora，是目前唯一已实现的连接器。
...
| 导出任务轮询间隔 | 2s | 飞书 Export API 状态检查间隔 |
| 导出任务最大等待 | ~60s | 导出任务超时时间 |
