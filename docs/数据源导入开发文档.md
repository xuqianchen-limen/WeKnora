...
- [连接器详解](#连接器详解)
  - [飞书 (Feishu)](#飞书-feishu)
  - [企业微信文档 (WeCom Doc)](#企业微信文档-wecom-doc)
- [定时调度](#定时调度)
- [关键参数与阈值](#关键参数与阈值)
- [错误处理](#错误处理)
- [扩展新连接器](#扩展新连接器)
...
从企业微信同步智能文档（文档、表格）和微盘文件到 WeKnora。支持智能文档分类同步和微盘空间文件递归下载。

#### 第一步：获取应用凭证

1. 登录 [企业微信管理后台](https://work.weixin.qq.com/wework_admin/frame#apps) → **应用管理**
2. 选择已有的自建应用，或点击 **创建应用** 新建一个
3. 在应用详情页记录以下信息：
   - **Corp ID**：在左侧栏 **我的企业** → **企业信息** 页面底部获取
   - **Secret**：在应用详情页点击 **查看** 获取

#### 第二步：配置回调 URL（解锁管理后台的 API 设置）

企微管理后台要求先配置「接收消息服务器 URL」→ 才能配置「企业可信 IP」→ 才能设置「可调用接口的应用」。WeKnora 内置了回调验证端点来解决这个前置依赖链。

1. 先在 WeKnora 中创建企微文档数据源（仅填 Corp ID 和 Secret，先保存）
2. 在管理后台 **应用管理** → 你的自建应用 → **接收消息** → 点击 **设置API接收**
3. 企微后台会显示 **Token** 和 **EncodingAESKey**（可点击「随机获取」自动生成）
4. 将 Token 和 EncodingAESKey 复制到 WeKnora 数据源配置的「回调 URL 配置」区域中，并保存
5. 在 WeKnora 中复制生成的回调 URL（格式：`https://你的域名/api/v1/wecom/callback/<数据源ID>`）
6. 将此 URL 粘贴到企微后台的「URL」字段中，点击「保存」
7. 企微会向该 URL 发送验证请求，WeKnora 自动完成验证

#### 第三步：授权应用调用文档 API

完成回调 URL 配置后，即可设置「可调用接口的应用」。如果出现 `errcode=48002 api forbidden` 错误，说明此步骤未完成。

**配置文档接口权限：**

1. 在管理后台左侧栏点击 **协作** → **文档**
2. 滚动到底部找到 **API** 区域
3. 在「可调用接口的应用」中添加你的自建应用

**配置微盘接口权限（可选，仅同步微盘文件时需要）：**

1. 在管理后台左侧栏点击 **协作** → **微盘**
2. 滚动到底部找到 **API** 区域
3. 在「可调用接口的应用」中添加你的自建应用

> **关于企业可信 IP**：完成回调 URL 配置后，「企业可信 IP」也被解锁了。如果调用时收到 `errcode=60020` 错误，说明需要在应用详情页底部的「企业可信 IP」中添加服务器 IP。

#### 第四步：在 WeKnora 中完善数据源配置

1. 回到 WeKnora 知识库设置页 → **数据源** 标签页
2. 编辑之前创建的企微文档数据源
3. 点击 **测试连接**，验证凭证和权限均已就位
4. 选择要同步的资源范围：
   - **全部文档**：同步所有智能文档
   - **文档**：仅同步文档类型
   - **表格**：仅同步表格类型
...
如需同时同步微盘空间中的文件（PDF、Word、Excel 等上传文件），需在数据源配置的 `settings` 中提供：
...
| 参数 | 说明 |
|------|------|
| `space_ids` | 微盘空间 ID 列表（从微盘设置页或 API 获取） |
| `userid` | 用于文件下载的操作者 userid（需有对应空间的下载权限） |

> **注意**：企业微信目前不提供「获取空间列表」API，因此空间 ID 需要手动配置。若不配置 `space_ids`，仅同步智能文档。
...
| Confluence | `base_url`, `username`, `api_token` | API Token |
| 语雀 (yuque) | `api_token`, `group_login` | API Token |
| GitHub | `access_token`, `owner`, `repo` | Personal Access Token |
| Google Drive | `credentials_json` | OAuth2 Service Account |
| OneDrive | `client_id`, `client_secret`, `tenant_id` | OAuth2 |
| 钉钉文档 (dingtalk) | `app_key`, `app_secret` | OAuth2 |
| 企业微信文档 (wecom_doc) | `corp_id`, `corp_secret` | access_token (自建应用) |
| Web 爬虫 (web_crawler) | `start_urls`, `max_depth` | 无需认证 |
| Slack | `bot_token` | Bot Token |
| IMAP | `host`, `port`, `username`, `password` | 密码认证 |
| RSS | `feed_urls` | 无需认证 |
...
企微文档连接器支持两种数据源：**智能文档**（企微原生在线文档/表格）和 **微盘文件**（上传的 PDF/Word/Excel 等）。
...
Corp ID + Corp Secret
        │
        ▼
  GET /cgi-bin/gettoken?corpid=xxx&corpsecret=xxx
        │
        ▼
  获取 access_token（有效期 7200 秒 / 2 小时）
        │
        ▼
  Token 缓存，提前 5 分钟刷新
```

- **认证类型**：企业自建应用，access_token
- **Token 有效期**：7200 秒（2 小时），内置缓存，提前 5 分钟自动刷新
- **API 地址**：`https://qyapi.weixin.qq.com`
- **Token 传递方式**：通过 URL 参数 `access_token=xxx`

#### 资源发现

`ListResources` 返回文档分类资源和可选的微盘空间：

**智能文档分类（始终返回）：**

| 资源 ExternalID | 名称 | 对应 API 参数 |
|----------------|------|-------------|
| `all_documents` | 全部文档 | `doc_type=0` |
| `documents` | 文档 | `doc_type=3` |
| `sheets` | 表格 | `doc_type=4` |

**微盘空间（仅当 `settings.space_ids` 配置时返回）：**

| 资源 ExternalID | 名称 | 类型 |
|----------------|------|------|
| `wedrive:<space_id>` | 微盘空间: {id} | `wedrive_space` |
...
```
POST /cgi-bin/wedoc/doc_list?access_token=xxx
Body: { "doc_type": 0, "limit": 50, "cursor": "" }

返回: { "doc_info_list": [{"docid": "...", "doc_name": "..."}], "has_more": true, "next_cursor": "..." }
```

**获取文档元数据（用于增量同步时间对比）：**

```
POST /cgi-bin/wedoc/get_doc_base_info?access_token=xxx
Body: { "docid": "xxx" }

返回: { "doc_base_info": { "docid": "...", "doc_name": "...", "create_time": 1700000000, "modify_time": 1700001000, "doc_type": 3 } }
```

每个文档映射为一个 `FetchedItem`：

| 字段 | 值 |
|------|------|
| `ExternalID` | docid |
| `Title` | doc_name |
| `URL` | `https://doc.weixin.qq.com/doc/{docid}` 或 `sheet/{docid}` |
| `ContentType` | `text/html` |
| `Metadata.channel` | `wecom` |
| `Metadata.doc_type` | 文档类型（3=文档, 4=表格） |
...
```
POST /cgi-bin/wedrive/file_list?access_token=xxx
Body: { "spaceid": "xxx", "sort_type": 5, "start": 0, "limit": 1000 }

返回: { "file_list": { "item": [...], "has_more": false } }
```
...
|-------------|------|---------|
| 1 | 文件夹 | 递归遍历子项 |
| 2 | 上传文件 | 通过 file_download 下载二进制内容 |
| 3 | 智能文档 | 通过 URL 导入 |
| 4 | 智能表格 | 通过 URL 导入 |
| 5 | 收集表 | 通过 URL 导入 |
...
```
POST /cgi-bin/wedrive/file_download?access_token=xxx
Body: { "fileid": "xxx", "userid": "admin_userid" }

返回: { "download_url": "https://...", "cookie_name": "...", "cookie_value": "..." }
```

下载时需携带返回的 Cookie 请求 `download_url` 获取文件内容。

#### 增量同步 (FetchIncremental)

增量同步基于文档 `modify_time` 对比：

```
1. 加载上次游标 (wecomCursor)
   └─ DocModifyTimes[docid] = 上次记录的修改时间

2. 对每个选中的资源类别：
   └─ 列举所有文档 (ListSmartDocuments)
   └─ 对每个文档获取元数据 (GetDocBaseInfo)

3. 对比变更:
   ├─ 新文档（游标中不存在）→ 加入变更列表
   ├─ modify_time 变更 → 加入变更列表
   ├─ modify_time 未变 → 跳过
   └─ 游标中存在但本次不存在 → 标记为 IsDeleted

4. 微盘空间：
   └─ 列举所有文件 (ListAllDriveFilesRecursive)
   └─ 对比 mtime，逻辑同上

5. 返回新游标（含本轮每个文档/文件的最新修改时间）
```
...
| 文件 | 职责 |
|------|------|
| `internal/datasource/connector/wecom/types.go` | WeCom API 类型定义、配置结构、常量（文档/微盘类型） |
| `internal/datasource/connector/wecom/client.go` | API 客户端：Token 管理、智能文档列表/详情、微盘文件列表/递归/下载 |
| `internal/datasource/connector/wecom/connector.go` | Connector 接口实现：Validate、ListResources、FetchAll、FetchIncremental |
