export default {
  ...
  datasource: {
    ...
    noResourcesDesc_wecom_doc: "请确认已在管理后台「协作→文档→API」中将应用添加为「可调用接口的应用」",
    ...
    guideStep1_wecom_doc: "在管理后台「协作→文档→API」中将自建应用添加为「可调用接口的应用」",
    guideStep2_wecom_doc: "确认应用「可见范围」包含了需要同步文档的成员或部门",
    guideStep3_wecom_doc: "填入 Corp ID 和 Secret 后点击重新加载",
    ...
    comingSoon: "即将支持",
    docHint: "在以下地址获取凭证：",
    wecomCallbackTitle: "回调 URL 配置（解锁「可调用接口的应用」设置）",
    wecomCallbackDesc: "企微管理后台要求配置「接收消息服务器URL」后才能设置「企业可信IP」和「可调用接口的应用」。在此填入 Token 和 EncodingAESKey（从企微后台获取），保存后将生成的回调 URL 粘贴到企微后台。",
    wecomCallbackUrl: "回调 URL（粘贴到企微后台「接收消息服务器URL」）",
    wecomCallbackSaveFirst: "请先保存数据源，保存后会生成回调 URL",
    copyUrl: "复制 URL",
    copied: "已复制",
    pleaseTestFirst: "请先测试连接",
    prereqTitle: "使用前请先完成以下配置（缺一不可）",
    prereqBarText: "首次使用？点击查看飞书应用配置指引",
    prereqBarText_wecom_doc: "首次使用？点击展开企微应用配置指引",
    prereqStep1Brief_wecom_doc: "配置回调 URL（解锁后续设置）",
    prereqStep1Desc_wecom_doc: "先保存数据源 → 展开下方「回调 URL 配置」→ 填入企微后台的 Token/EncodingAESKey → 将生成的 URL 粘贴到企微后台「接收消息服务器URL」",
    prereqStep2Brief_wecom_doc: "授权应用调用文档 API",
    prereqStep2Desc_wecom_doc: "管理后台 → 协作 → 文档 → 滚动到底部 API 区域 → 将自建应用添加到「可调用接口的应用」",
    prereqStep3Brief_wecom_doc: "确认应用可见范围",
    prereqStep3Desc_wecom_doc: "管理后台 → 应用管理 → 自建应用详情 → 确保「可见范围」包含需要同步文档的成员或部门",
